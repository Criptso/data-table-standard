    ...
    if (narrowed !== 0)
      ...
      const msg = await page.evaluate(sel => {
        ...
        const el = [...t.querySelectorAll("*")].find(e =>
          /no (rows|records|results|matching)|niciun r[âa]nd|nicio [îi]nregistrare/i.test(e.textContent)
      ...
      if (h) {
        ...
        const range = await page.evaluate(sel => {
          ...
          return { dates: m.querySelectorAll("input[type=date]").length,
                   periods: [...m.querySelectorAll("button")].filter(b => /last|day|month|year|ultim|zile|luni|an/i.test(b.textContent)).length };
        }, SEL.menu);
