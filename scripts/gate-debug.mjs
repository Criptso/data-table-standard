...
// ── 11 + 14. dates ─────────────────────────────────────────────────────────────────
{
  ...
  if (!dateCells.length) skip(14, "date formatting", "no date column detected");
    ...
    if (await has(SEL.menuHandle)) {
      ...
      console.log("DEBUG dateCol i=", dateCells[0].i, "mapped=", JSON.stringify(mapped), "handles=", handles.length,
                  "cols=", JSON.stringify(await handleCols()));
      if (h) {
        console.log("DEBUG menu open before click:", await page.evaluate(sel => !!document.querySelector(sel), SEL.menu));
        const bb = await h.boundingBox();
        console.log("DEBUG handle box:", JSON.stringify(bb));
        console.log("DEBUG topmost at that point:", await page.evaluate((x, y) => {
          const el = document.elementFromPoint(x, y);
          return el ? `${el.tagName}.${(el.className.toString()||'').slice(0,50)} testid=${el.getAttribute('data-testid')}` : null;
        }, bb.x + bb.width / 2, bb.y + bb.height / 2));
        await h.click();
        await sleep(300);
        console.log("DEBUG menu open after click:", await page.evaluate(sel => {
          const m = document.querySelector(sel);
          return m ? m.getAttribute("data-column") || m.className || "menu" : null;
        }, SEL.menu));
        console.log("DEBUG handles state:", await page.evaluate(() => [...document.querySelectorAll("[data-cmenu]")].map(b => ({
          id: b.getAttribute("data-testid"), expanded: b.getAttribute("aria-expanded"),
          x: Math.round(b.getBoundingClientRect().x), y: Math.round(b.getBoundingClientRect().y),
          w: Math.round(b.getBoundingClientRect().width),
        }))));
        console.log("DEBUG portals:", await page.evaluate(() => [...document.querySelectorAll("[role=dialog], .column-menu")].map(e => e.className.slice(0,40))));
        console.log("DEBUG scrollLeft:", await page.evaluate(() => {
          const b = document.querySelector('[data-slot="table-container"]');
          return b ? { left: b.scrollLeft, w: b.clientWidth, sw: b.scrollWidth } : null;
        }));
