...
const URL = 'http://127.0.0.1:3428/e2e/login?name=cristi&pin=2468&next=/admin/movements';
const b = await puppeteer.launch({ executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', headless: 'new',
  userDataDir: '/Volumes/2Tm2/TEMP/casestock-verify/t29/chrome-probe3', args: ['--no-first-run','--no-default-browser-check'] });
...
const opened = await p.evaluate(() => {
  const th = [...document.querySelectorAll('thead th')].find(t => /DATE|DATA/i.test(t.textContent));
